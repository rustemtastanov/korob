...
(function() {
  ...
  var browser = detect();
  if (browser) $("html").addClass("ie ie-" + browser);
...
(function() {
  ...
  var load = function() {
    ...
    // $("[data-role=\"lazybg\"]").filter(":not(.loaded)").children("div").Lazy({
    //   scrollDirection: "vertical",
    //   visibleOnly: true,
    //   afterLoad: function(element) {
    //     $(element).parent().addClass("loaded");
...
(function() {
	...
	$(document)
		.bind("showCatalogNavEvent", function() {
			set();
		})
...
(function() {
	...
	var $sup 		= $("[data-role=\"header-sup\"]");
	var $sub 		= $("[data-role=\"header-sub\"]");

	var scroll = function() {
		var y = $win.scrollTop();
		var offset = $sup.height();
		if (y>=offset) {
...
(function() {
	...
	var show = function() {
		...
		$(document).trigger("showCatalogNavEvent");
	};

	var hide = function() {
		showed = false;
		$body.removeClass("show--catalog");
		$nav.trigger("hideNavEvent");
		$(document).trigger("hideCatalogNavEvent");
	};

	$nav.find(".li-drop").each(function() {
		var $li 	= $(this);
		var $link = $li.children("a");
		var $drop = $li.children("ul");
		var opened = $li.hasClass("toggled");
		...
		var close = function() {
			...
			$drop.slideUp(300, function() {
				$(this)
					.removeAttr("style")
					.find(".toggled").removeClass("toggled").find("ul").removeAttr("style");
			});
		};
		...
		$nav.bind("hideNavEvent", function() {
			if (opened) close();
		});
	});
...
(function() {
	...
	var $body 	= $("body");
	var $nav 		= $("[data-nav=\"footer\"]");

	$nav.find("[data-role=\"toggle\"]").each(function() {
		var $drop = $(this);
		var $btn 	= $drop.parent("div").find("[data-button=\"toggle\"]");
		var toggled = $btn.hasClass("toggled");
		...
		var close = function(silent_mode) {
			...
			} else $drop.slideUp(300, function() {
							$(this).removeAttr("style");
						});
		};

		var check = function() {
			if (DEVICE!="phone" && toggled) close(true);
		};
...
(function() {
	...
	var $section = $("[data-role=\"offers\"]");

	if (!$section["length"]) return false;

	var $slider 	= $section.find("[data-role=\"carousel\"]");
	var $sliderIn = $section.find("[data-role=\"carousel-in\"]");
	...
	var check = function() {
		if ((DEVICE=="tablet"||DEVICE=="phone") && !slider) init();
		if (DEVICE!="tablet" && DEVICE!="phone" && slider) destroy();
	};
...
(function() {
	...
	$("[data-role=\"seo\"]").each(function() {
		var $div = $(this);
		var $toggle = $div.find("[data-role=\"toggle\"]");
		var $btn = $div.find("[data-button=\"toggle\"]");
